...
- **RAG chat** (`POST /chat`, `ChatEndpoint`): kimlik doğrulaması gerektirmeyen, dış API'lere bağımlı bir uçtur — `ChatService` (`IRagChunkRepository` üzerinden `RagChunks` tablosunu okur), soruyu `IVoyageEmbeddingClient` (`Services/External/VoyageEmbeddingClient`) ile embed eder, `EmbeddingVector.CosineSimilarity` ile in-memory en yakın `TopK` chunk'ı bulur, cevabı `IClaudeChatClient` (`Services/External/ClaudeChatClient`) ile üretir. `RagChunk` verisi `docs/*.md` makalelerinden Python pipeline'ı (`rag/ingest.py` + `rag/embed.py`, çıktı `rag/rag.db`) tarafından üretilir ve API başlangıcında `RagChunkSeeder` tarafından `AppDbContext.RagChunks`'a import edilir (`rag/rag.db` eksik/bozuksa seeder hatayı yutar, API başlamaya devam eder, sadece `/chat` 503 döner). `/chat`, IP başına dakikada 10 istekle sınırlanan bir rate limiter (`"chat"` policy'si) ile korunur ve mesaj uzunluğu 2000 karakterle sınırlıdır.
